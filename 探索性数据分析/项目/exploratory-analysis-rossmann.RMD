...
# numeric->int
sp[c('CompetitionDistance', 'Promo2OpenDays')] <- lapply(sp[c('CompetitionDistance', 'Promo2OpenDays')], as.integer)
# factor->date
...
median_sales_by_store <- ddply(sp, 'Store', summarise, sales.median = median(Sales))
ggplot(median_sales_by_store, aes(sales.median)) + 
  geom_histogram(aes(y=..density..), binwidth = 360, fill = I('cornflowerblue'), col = I('white')) +
  stat_density(aes(y=..density..), geom='line', linetype="dashed", size=1, col = I('darkorange')) +
...
median_sales_by_store_type <- ddply(sp, 'StoreType', summarise, sales.median = median(Sales))

ggplot(sp, aes(x = Sales)) + 
  geom_density(aes(fill = StoreType), alpha = 0.4, size = 0.8) + 
  geom_vline(data = median_sales_by_store_type, aes(xintercept = sales.median, color = StoreType), linetype = 'dashed', size = 0.8) + 
...
median_sales_by_assortment <- ddply(sp, 'Assortment', summarise, sales.median = median(Sales))

ggplot(sp, aes(x = Sales)) + 
  geom_density(aes(fill = Assortment), alpha=0.4, size=0.8) + 
  geom_vline(data = median_sales_by_assortment, aes(xintercept = sales.median, color = Assortment), linetype = 'dashed', size = 0.8) + 
...
median_sales_by_promo <- ddply(sp, 'Promo', summarise, sales.median = median(Sales))
median_sales_by_promo2 <- ddply(sp, 'Promo2', summarise, sales.median = median(Sales))

p1 <- ggplot(sp, aes(x = Sales)) + 
  geom_density(aes(fill = factor(Promo)), alpha = 0.4, size = 0.8) + 
  geom_vline(data = median_sales_by_promo, aes(xintercept = sales.median, color = factor(Promo)), linetype = 'dashed', size = 0.8) + 
...
p2 <- ggplot(sp, aes(x = Sales)) + 
  ...
  geom_vline(data = median_sales_by_promo2, aes(xintercept = sales.median, color = factor(Promo2)), linetype = 'dashed', size = 0.8) + 
  scale_color_manual(values = c(I('royalblue'), I('gold1'))) + 
  scale_fill_manual(values = c(I('royalblue'), I('gold1'))) +
...
median_sales_by_holiday <- ddply(sp, 'IsHoliday', summarise, sales.median = median(Sales))

ggplot(sp, aes(x = Sales)) + 
  geom_density(aes(fill = factor(IsHoliday)), alpha = 0.4, size = 0.8) + 
  geom_vline(data = median_sales_by_holiday, aes(xintercept = sales.median, color = factor(IsHoliday)), linetype = 'dashed', size = 0.8) +
  scale_color_manual(values = c(I('royalblue'), I('gold1'))) + 
  scale_fill_manual(values = c(I('royalblue'), I('gold1'))) +
...
median_sales_by_dow <- ddply(sp, 'DayOfWeek', summarise, sales.median = median(Sales))

ggplot(sp, aes(x = Sales)) + 
  geom_density(aes(fill = factor(DayOfWeek)), alpha = 0.4, size = 0.8) + 
  geom_vline(data = median_sales_by_dow, aes(xintercept = sales.median, color = factor(DayOfWeek)), linetype = 'dashed', size = 0.8) + 
...
p1 <- ggplot(subset(sp, Store %in% sample_stores), aes(x = factor(Store), y = Sales)) + 
  geom_boxplot() +
  ggtitle('Sales by Sample Store Box Plot') + 
  xlab('Store')

p2 <- ggplot(subset(sp, Store %in% sample_stores), aes(x = factor(Store), y = Sales)) + 
  geom_violin() +
...
median_sales_by_month_store <- ddply(sp, c('Month', 'Store'), summarise, sales.median = median(Sales))

ggplot(subset(median_sales_by_month_store, Store %in% sample_stores), aes(x = as.integer(Month), y = sales.median, color = factor(Store), shape = factor(Store))) +
  geom_point(size=3) +
  geom_line(size=1) +
...
median_sales_by_dow_store <- ddply(sp, c('DayOfWeek', 'Store'), summarise, sales.median = median(Sales))

ggplot(subset(median_sales_by_dow_store, Store %in% sample_stores), aes(x = as.integer(DayOfWeek), y = sales.median, color = factor(Store), shape = factor(Store))) +
  geom_point(size=3) +
  geom_line(size=1) +
...
ggplot(subset(sp, CompetitionOpenDays>0 & CompetitionOpenDays<=365), aes(x = CompetitionOpenDays, y=Sales)) +
    geom_smooth() + 
...
median_sales_cd_by_store <- ddply(sp, 'Store', summarise, sales.median = median(Sales), cd = median(CompetitionDistance))
...
median_sales_by_month_assortment_promo <- ddply(sp, c('Month', 'Assortment', 'Promo'), summarise, sales.median = median(Sales))

ggplot(median_sales_by_month_assortment_promo, aes(x = as.integer(Month), y = sales.median, color = Assortment)) +
  geom_point() +
  geom_line() +
  scale_x_continuous(limits = c(1, 12), breaks = seq(0, 12, 2)) + 
...
median_sales_by_month_dow_store_type <- ddply(sp, c('Month', 'DayOfWeek', 'StoreType'), summarise, sales.median = median(Sales))

ggplot(median_sales_by_month_dow_store_type, aes(x = as.integer(Month), y = sales.median, color = StoreType)) +
  geom_point() +
  geom_line() +
  scale_x_continuous(limits = c(1, 12), breaks = seq(0, 12, 2)) + 
...
ggplot(subset(sp, Store == 262), aes(x = Date, y = Sales, color = factor(DayOfWeek == 7), shape = factor(DayOfWeek == 7))) + 
...
heat_sp$StoreType <- revalue(x = heat_sp$StoreType, c('a' = 1, 'b' = 2, 'c' = 3, 'd' = 4))
heat_sp$Assortment <- revalue(x = heat_sp$Assortment, c('a' = 1, 'b' = 2, 'c' = 3))
...
ggplot(data = melted_cormat, aes(Var2, Var1, fill = value)) +
 ...
 scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0, limit = c(-1,1), space = "Lab", name="Sales Prediction\nCorrelation") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, size = 12, hjust = 1)) +
  coord_fixed() +
  theme(axis.title.x = element_blank(), axis.title.y = element_blank(), panel.grid.major = element_blank(), panel.border = element_blank(), panel.background = element_blank(), axis.ticks = element_blank(), legend.justification = c(1, 0), legend.position = c(0.6, 0.7), legend.direction = "horizontal") +
...
### 5. 绘图三
```{r echo=FALSE, results='hide', warning=FALSE, Core_Features_Heatmap2}
# Heatmap
ggplot(data = melted_cormat, aes(Var2, Var1, fill = value)) +
 geom_tile(color = "white") +
 scale_fill_gradient2(low = "blue", high = "red", mid = "white", midpoint = 0, limit = c(-1,1), space = "Lab", name="Sales Prediction\nCorrelation") +
